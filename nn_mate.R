nn_mate <- function(nn1, nn2, disable_p = 0.8){
  ...
  connect_df$fitness_pick <- ifelse((nn1$fitness == nn2$fitness), 0, ifelse(nn1$fitness > nn2$fitness,1,2))
  ...
  return(list(node_df = new_node_df, connect_df = new_connect_df))
